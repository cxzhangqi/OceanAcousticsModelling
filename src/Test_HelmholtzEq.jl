...
begin
	...
	zBty(r) = zMax - (zMax - zMin)*exp(-(r - rPeak)^2/2e5)
end
...
begin
	rTemp = range(0, rMax, length = 100)
	zTemp = range(0., zMax, length = 100)
end
...
C(r) = [1 zAti zAti^2
	 1 (zAti + zBty(r))/2 ((zAti + zBty(r))/2)^2
	 1 zBty(r) zBty(r)^2]
...
begin
	plot(sol, vars = (1,2), yaxis = :flip)
	plot!(rTemp, zBty.(rTemp))
end
